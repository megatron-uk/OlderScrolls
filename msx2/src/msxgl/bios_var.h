...
extern const CassetteIO __at(M_CS120) g_CS120;

#define M_CS240	0xF401	// Cassette I/O parameters to use for 2400 baud
extern const CassetteIO __at(M_CS240) g_CS240;
...
extern const u8 __at(M_LOW) g_LOW[2];
...
extern const u8 __at(M_HIGH) g_HIGH[2];
...
extern c8 __at(M_FNKSTR) g_FNKSTR[10][16];
...
extern const RS232Queue __at(M_QUETAB) g_QUETAB[4];
...
extern const u8 __at(M_QUEBAK) g_QUEBAK;
...
extern const u8 __at(M_MODE) g_MODE;
...
// bit 0-2: Logical operation on kanji font draw.
	  ...
	  // 3 for XOR
	  // 4 for NOT
extern const u8 __at(M_NORUSE) g_NORUSE;
...
extern const RS232Hook __at(M_MEXBIh) g_MEXBIh;

#define M_OLDSTT	0xFB0C	// 5	Hook called by the RS-232C.
extern const RS232Hook __at(M_OLDSTT) g_OLDSTT;

#define M_OLDINT	0xFB0C	// 5	Hook called by the RS-232C.
extern const RS232Hook __at(M_OLDINT) g_OLDINT;

#define M_DEVNUM	0xFB16	// 1	Byte offset. (RS-232C)
extern const u8 __at(M_DEVNUM) g_DEVNUM;
...
extern const u8 __at(M_ENSTOP) g_ENSTOP;

#define M_BASROM	0xFBB1	// 1	<>0 if basic is in rom. (CTRL+STOP disabled)
extern const u8 __at(M_BASROM) g_BASROM;
...
// Bit 5 - 0 Unused
extern const u8 __at(M_DRWFLG) g_DRWFLG;

#define M_DRWSCL	0xFCBC	// 1	Used by the instruction DRAW (DrawScaling).
extern const u8 __at(M_DRWSCL) g_DRWSCL;
...
// 2 = 180° rotation
// 3 = 270° rotation
extern const u8 __at(M_DRWANG) g_DRWANG;

#define M_MNROM		0xFCC1	// 1
extern const u8 __at(M_MNROM) g_MNROM;
...
extern const u8 __at(M_EXPTBL) g_EXPTBL[4];
...
extern const u8 __at(M_SLTTBL) g_SLTTBL[4];
...
// Bits 4~0 = Unused
extern const u8 __at(M_SLTATR) g_SLTATR[64];
...
extern u8 __at(M_SLTSL) g_SLTSL;

#endif // ifndef BIOS_VAR_H